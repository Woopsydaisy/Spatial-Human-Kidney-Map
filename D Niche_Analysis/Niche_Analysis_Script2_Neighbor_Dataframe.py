...
sc.settings.figdir = "/home/bcd/revision_nature/neighbors_neighborhood/plots_script3"  # Update this path as needed
...
for i in adata.obs["unique_sample_identifier"].unique():
    ...
    for pop in adata_sample.obs['annotation_updated'].unique():
        neighbor_type_indices = np.where(adata_sample.obs['annotation_updated'] == pop)[0]
...
df_concat.to_csv('/home/bcd/revision_nature/neighbors_neighborhood/neighborhood_dataframe.csv', index=True)

df_concat=pd.read_csv('/home/bcd/revision_nature/neighbors_neighborhood/neighborhood_dataframe.csv')
print(df_concat)
